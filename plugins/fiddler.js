...
plugin.onKeybindDown("Toggle Godmode", () => {
	if (game?.currentScene?.myAnimal != null && game.currentScene.myAnimal.fishLevelData.fishLevel === 121)
		toggle = !toggle;
});
...
function tick() {
	try {
		...
		if (game?.currentScene?.myAnimal?.fishLevelData?.fishLevel !== 121)
			return toggle = false;
		if (game.currentScene.myAnimal.fishData.overHealth >= plugin.getSetting("Limit")) return;
		if (!toggle) return;
		game.socketManager.sendBytePacket(blockyfish.encodeBytePacket(gameState.token._value, blockyfish.ActionPacket.FiddlerScore));
	} catch { }
}
interval = setInterval(tick, delay);